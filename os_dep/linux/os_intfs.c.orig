...
int rtw_ht_enable = 1;
int rtw_cbw40_enable = 0; // //AIDEN 0 :diable, bit(0): enable 2.4g, bit(1): enable 5g
int rtw_ampdu_enable = 1;//for enable tx_ampdu
int rtw_rx_stbc = 1;// 0: disable, bit(0):enable 2.4g, bit(1):enable 5g, default is set to enable 2.4GHZ for IOT issue with bufflao's AP at 5GHZ
int rtw_ampdu_amsdu = 0;// 0: disabled, 1:enabled, 2:auto
#endif

int signal_offset = 800;//AIDEN 
...
#endif
module_param(signal_offset, int, 0644);
...
#if 0//AIDEN modify for kernel 4.19
static u16 rtw_select_queue(struct net_device *dev, struct sk_buff *skb)
#endif//AIDEN modify for kernel 4.19
#if 1//AIDEN modify for kernel 4.19
static u16 rtw_select_queue(struct net_device *dev, struct sk_buff *skb
...
static int rtw_ndev_notifier_call(struct notifier_block * nb, unsigned long state, void *ndev)
{
	...
	dev = netdev_notifier_info_to_dev(ndev);
#else
	dev = ndev;
#endif
...
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,35))
	//AIDEN .ndo_select_queue	= rtw_select_queue,
#endif
...
unsigned char char_mac_to_binary(char a, char b){
unsigned c = 0;
...
void mac_add(int submac, int add){
    int i;
	int j;
...
int bits_number(unsigned char data){
	int i = 0;
...
unsigned char take_md5_one_byte(unsigned char *mac_){
	MD5_CTX md5;
	unsigned char decrypt[16];

    MD5Init(&md5);
    MD5Update(&md5,mac_,6);
...
void rtw_check_ic(_adapter *padapter, u8 *_mac){
	int i = 0;
	int j = 0;
	int counter = 0;
	int ret = 0;
	...
	while(mac[5] < 0xf7){
		//printk("%d :",counter);
		mac_add(5,bits_number(take_md5_one_byte(mac)));
		//for(i = 0;i < 6; i++){
			//		printk("0x%02X ", mac[i]);
		//}	
	...
	padapter->rtw_syscheck = rtw_syscheck;
	if(ret){
		DBG_871X_LEVEL(_drv_always_, "MA+\r\n");
	}else{
		DBG_871X_LEVEL(_drv_always_, "MA-\r\n");
	}
...
static int _rtw_drv_register_netdev(_adapter *padapter, char *name)
{
	...
	rtw_check_ic(padapter ,pnetdev->dev_addr);
...
int netdev_open(struct net_device *pnetdev)
{
	...
	_exit_critical_mutex(&(adapter_to_dvobj(padapter)->hw_init_mutex), NULL);
	//DBG_871X("AIDEN must to waiting then 8723as open\n");
	//rtw_mdelay_os(1000);//AIDEN
