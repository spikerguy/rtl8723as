...
extern int signal_offset;//AIDEN
struct cfg80211_bss *rtw_cfg80211_inform_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	#endif
	#if 1//AIDEN
	if(notify_signal + signal_offset > 0)
	{
		notify_signal = 0;
	}
	else
	{
		notify_signal = notify_signal + signal_offset;
	}
	#endif
	#if 0//AIDEN
	DBG_8192C(MAC_FMT"     %d      %d     %s\n", 
	    MAC_ARG(pnetwork->network.MacAddress),
	    freq,
	    notify_signal,
...
//	DBG_871X("AIDEN waiting for scan 1s\n");
...
int p2p_find_stop = _FALSE;//AIDEN
