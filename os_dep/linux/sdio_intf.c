...
static char firmware_path[MOD_PARAM_PATHLEN];
static char nvram_path[MOD_PARAM_PATHLEN];
static char config_path[MOD_PARAM_PATHLEN];
int reint_flag = 0;
...
#if 1//ADIEN reinit
struct semaphore reinit_sem;
int driver_remove = 0;


void wifi_reg_on(bool onoff){
#if 0//for AML
        extern_wifi_set_enable(onoff);
#endif//for AML

#if 1//for RK
	rockchip_wifi_power(onoff);
#endif//for RK

#if 0//for allwinner
	sunxi_wlan_set_power(onoff);
#endif//for allwinner
}

void sdio_carddetect(int flag){
#if 0//for AML
        sdio_reinit();
#endif//for AML

#if 1//for RK
	rockchip_wifi_set_carddetect(flag);
#endif//for RK
#if 0//for allwinner
...
extern void reinit_up(void){
	DBG_871X_LEVEL(_drv_always_, "reinit_up\n");
	up(&reinit_sem);
}

static int reinit_drv_entry(void *data){
	int ret = 0;
	
	DBG_871X_LEVEL(_drv_always_, "8723as module reint enter\n");

	sema_init(&reinit_sem, 0);
while(1){
	down(&reinit_sem);
	sema_init(&reinit_sem, 0);
	if(driver_remove){
	...
	reint_flag = 1;
	DBG_871X_LEVEL(_drv_always_, "AIDEN reinit_drv_entry\n");

	DBG_871X_LEVEL(_drv_always_, "+sdio_unregister_driver\n");
...
exit:
	DBG_871X_LEVEL(_drv_always_, "module reint ret=%d\n", ret);

	}

	DBG_871X_LEVEL(_drv_always_, "8723as module reint exit\n");
	return 0;
}
...
static int __init rtw_drv_entry(void)
{
	...
	DBG_871X_LEVEL(_drv_always_, "module init start Aiden to debug 2\n");
	...
	//AIDEN add begin
	while(sdio_drvpriv.drv_registered != _TRUE){
		mdelay(1000);
		if(retryCounter == 3){
			ret = -1;
			break;
		}
		if(sdio_drvpriv.drv_registered == _TRUE){
			ret = 0;
			break;
		}
		retryCounter++;
		DBG_871X_LEVEL(_drv_always_, "init retry %d\n",retryCounter);
	}
	...
	kthread_run(reinit_drv_entry, NULL, "REINIT_THREAD");
	//AIDEN add end
...
static void __exit rtw_drv_halt(void)
{
	...
	while(sdio_drvpriv.drv_registered == _FALSE || reint_flag){
		DBG_871X_LEVEL(_drv_always_, "%s reinit not ready %d %d \n", __FUNCTION__, sdio_drvpriv.drv_registered, reint_flag);
		msleep(100);
	}
...
#endif
{
    ...
    printk("=======================================================\n");
    printk("Realtek 8723AS SDIO WiFi driver (Powered by Rockchip) init.\n");
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 1, 0))
