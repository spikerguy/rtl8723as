...
//		return;//AIDEN to fix p2p not discovery

	
	if(p2p_find_stop && p2p_find){
		DBG_871X("AIDEN p2p_find_stop\n");
		p2p_find = _FALSE;
		p2p_find_stop = _FALSE;
		return;
	}
	if(p2p_find_stop && !p2p_find){
		p2p_find_stop = _FALSE;
...
static bool rtw_chk_p2pie_op_ch_with_buddy(_adapter *padapter, const u8 *frame_body, u32 len)
{
...
#if 1//for Aiden debug
		int i = 0;

	    printk("AIDEN buddy_ch = %d \n", buddy_ch);
		printk("AIDEN check frame data start: \n");
		for(i = 0; i < len; i++){
			if(i % 20 == 0){
				printk("\n");
			}
			printk("%02X ", frame_body[i]);
		}
		printk("\n\nAIDEN check frame data end \n");
#endif

	ies = (u8*)(frame_body + _PUBLIC_ACTION_IE_OFFSET_);
	ies_len = len - _PUBLIC_ACTION_IE_OFFSET_;
#if 1 //for Aiden debug
	printk("AIDEN ies first data:%02X ies_len:%d\n", ies[0], ies_len);
#endif
	p2p_ie = rtw_get_p2p_ie( ies, ies_len, NULL, &p2p_ielen );
	printk("AIDEN p2p_ie:%02X p2p_ielen:%d\n", p2p_ie[0], p2p_ielen);
	...
	while (p2p_ie) {
		...
		pattr = NULL;
		if((pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_OPERATING_CH, NULL, (uint*)&attr_contentlen))!=NULL) {
			printk("AIDEN *(pattr+4) = %d buddy_ch = %d\n", *(pattr+4), buddy_ch);
			//if (*(pattr+4) == buddy_ch) {
...
_func_enter_;
	...
	DBG_871X("p2p_protocol_wk_hdl cmdType:%d \n", intCmdType);
